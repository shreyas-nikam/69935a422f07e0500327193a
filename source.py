...
def generate_data(
    ...
    """
    Generates a synthetic dataset for credit default prediction and splits it.
    ...
    Args:
        ...
        n_classes (int): Number of classes (binary classification).
        weights (List[float], optional): Class weights to simulate imbalance. Defaults to [0.9, 0.1].
    ...
    feature_names = [f'feature_{i}' for i in range(n_features - 5)] + \
                    ['fico_score', 'dti', 'income', 'loan_amount', 'employment_length']
...
def simulate_black_box_model(
    ...
    mock_black_box_model = xgb.XGBClassifier(
        ...
        eval_metric='logloss',
        random_state=random_state,
        scale_pos_weight=(y_train == 0).sum() / (y_train == 1).sum() # Addresses class imbalance
    )
...
def stability_test(mock_black_box_model: xgb.XGBClassifier, X_test: pd.DataFrame, noise_level: float = 0.01, n_trials: int = 10) -> pd.DataFrame:
    ...
    """
    baseline_probs = mock_black_box_model.predict_proba(X_test.to_numpy())[:, 1]
    ...
    for trial in range(n_trials):
        ...
        X_noisy = X_noisy.clip(lower=X_test.min().to_numpy(), upper=X_test.max().to_numpy()) # Simple clipping to keep values within reasonable range

        noisy_probs = mock_black_box_model.predict_proba(X_noisy.to_numpy())[:, 1]
...
def generate_validation_plots(
    ...
    challenger_tiers = define_risk_tiers(challenger_probs, risk_tier_thresholds)
    tier_cm = confusion_matrix(primary_tiers, challenger_tiers)

    plt.figure(figsize=(7, 6))
    sns.heatmap(tier_cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=[f'Tier {i}' for i in range(len(risk_tier_thresholds) + 1)],
    ...
    status_mapping_for_heatmap = {'FAIL': 0, 'CONDITIONAL': 1, 'PASS': 2, 'N/A': 0}
    numeric_scores_for_heatmap = [status_mapping_for_heatmap.get(s, 0) for s in scorecard_status]
    
    # Create a DataFrame suitable for heatmap (1 row, N columns)
    df_for_heatmap = pd.DataFrame([numeric_scores_for_heatmap], columns=criteria_labels)
...
def run_model_validation(
    ...
    benchmark_results = challenger_benchmark(
        X_train, y_train, X_test, y_test, primary_probs, reproduced_metrics['auc'],
